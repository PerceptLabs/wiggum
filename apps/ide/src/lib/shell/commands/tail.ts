...
/**
 * tail - Output last N lines of a file
...
export class TailCommand implements ShellCommand {
  ...
  async execute(args: string[], options: ShellOptions): Promise<ShellResult> {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '-n' && i + 1 < args.length) {
        ...
        if (isNaN(numLines) || numLines < 0) {
          return { exitCode: 1, stdout: '', stderr: `tail: invalid number of lines: '${args[i + 1]}'` }
        }
        i++
      } else if (arg.startsWith('-n')) {
        numLines = parseInt(arg.slice(2), 10)
        if (isNaN(numLines) || numLines < 0) {
          return { exitCode: 1, stdout: '', stderr: `tail: invalid number of lines: '${arg.slice(2)}'` }
        }
      } else if (arg.match(/^-\d+$/)) {
    ...
    if (files.length === 0 && stdin !== undefined) {
      ...
      // Remove trailing empty line from split if original ended with newline
