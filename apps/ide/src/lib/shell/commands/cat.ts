...
export class CatCommand implements ShellCommand {
  ...
  async execute(args: string[], options: ShellOptions): Promise<ShellResult> {
    ...
    for (const file of files) {
      // Virtual file: @wiggum/stack component listing
      if (file === '@wiggum/stack' || file === '@wiggum/stack/index' || file === '@wiggum/stack/index.ts') {
        outputs.push(getStackExports())
        continue
      }
 ...
 */
function getStackExports(): string {
  return `@wiggum/stack — Available exports
