 ...
 * Writes: packages/planning/src/generated-types.ts, generated-values.ts
 *
...
const valuesOutputFile = resolve(__dirname, '../src/generated-values.ts')
...
function formatArray(
  name: string,
  values: string[],
  opts?: { comment?: string, perLine?: number }
): string {
  const perLine = opts?.perLine ?? 4
  const lines: string[] = []
  if (opts?.comment) lines.push(`/** ${opts.comment} */`)
 ...
 */`,
  ...
  // Also generate runtime value arrays for plan validation
  const valueSections = [
    `/**
 * AUTO-GENERATED — DO NOT EDIT
 *
 ...
 */`,
    '',
    formatArray('MOOD_NAMES', moods, { comment: `${moods.length} personality moods` }),
    '',
    formatArray('PRESET_NAMES', presets, { comment: `${presets.length} curated presets` }),
    '',
    formatArray('PATTERN_NAMES', patterns, { comment: `${patterns.length} geometry patterns` }),
    '',
    formatArray('FONT_NAMES', fonts, { comment: `${fonts.length} curated fonts` }),
    '',
    formatArray('SHADOW_PROFILES', shadows, { comment: `${shadows.length} shadow profiles` }),
    '',
    formatArray('RADIUS_STOPS', radii, { comment: `${radii.length} radius stops` }),
    '',
    formatArray('GUMDROP_NAMES', gumdrops, {
      comment: `${gumdrops.length} gumdrops`,
      perLine: 5,
    }),
    '',
    formatArray('STACK_COMPONENTS', stackComponents, {
      comment: `${stackComponents.length} stack components`,
      perLine: 5,
    }),
    '',
  ]

  const valuesOutput = valueSections.join('\n')
  writeFileSync(valuesOutputFile, valuesOutput)
  console.log(`Wrote ${valuesOutputFile} (${valuesOutput.length} bytes)`)
}
